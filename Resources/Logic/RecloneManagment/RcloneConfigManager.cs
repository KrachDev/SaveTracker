...
namespace SaveTracker.Resources.Logic.RecloneManagment
{
    public class RcloneConfigManager
    {
      ...
      RcloneExecutor rcloneExecutor = new RcloneExecutor();
  ...
  public Task<bool> IsValidConfig(string path, CloudProvider provider) {
    string providerName = cloudProviderHelper.GetProviderConfigName(provider);
    string providerType = cloudProviderHelper.GetProviderConfigType(provider);
    string displayName = cloudProviderHelper.GetProviderDisplayName(provider);
    ...
    try {
      ...
      if (cloudProviderHelper.RequiresTokenValidation(provider)) {
  ...
  public async Task CreateConfig(IPlayniteAPI api, CloudProvider provider) {
    string providerName = cloudProviderHelper.GetProviderConfigName(provider);
    string providerType = cloudProviderHelper.GetProviderType(provider);
    string displayName = cloudProviderHelper.GetProviderDisplayName(provider);
    ...
    try {
      ...
      var result = await rcloneExecutor.ExecuteRcloneCommand(
