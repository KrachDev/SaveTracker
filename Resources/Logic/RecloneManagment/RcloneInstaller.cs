...
namespace SaveTracker.Resources.Logic.RecloneManagment
{
    ...
    {  
      RcloneConfigManager  _rcloneConfigManager = new RcloneConfigManager();
...
public async Task RcloneCheckAsync(IPlayniteAPI api) {
    ...
    try {
        ...
        var selectedIndex = UploaderSettings.Settings.SelectedProviderIndex;
        if (!Enum.IsDefined(typeof(CloudProvider), selectedIndex)) {
            DebugConsole.WriteError($"Invalid CloudProvider index: {selectedIndex}");
            return;
        }

        var provider = (CloudProvider)selectedIndex;
        ...
        DebugConsole.WriteKeyValue("Selected Provider", selectedIndex.ToString());
  ...
  public async Task DownloadAndInstallRclone(IPlayniteAPI api) {
  ...
  public Task ExtractRclone(string zipPath) {
  ...
  public async Task<string> GetRcloneVersion() {
    try {
      var result = await _executor.ExecuteRcloneCommand("version --check=false", TimeSpan.FromSeconds(10));
      if (result.Success) {
